 ...
 * @property {ReturnType<import('util').promisify<RedisClient["setnx"]>>} setnx
    ...
    const run = async () => {
      ...
      try {
        await this.runOperation(async () => {
          const locked = await this.expirationWorker.redis.promises.setnx(
            key,
            "OK"
          );
          if (locked) {
            await this.expirationWorker.redis.promises.pexpire(
              key,
              /** @type {number} */ (this.ttl)
            );
