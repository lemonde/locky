...
class Locky extends EventEmitter {
  ...
  async getLockerPromise(resource) {
    return this.runOperation(async () => {
      return this.redis.promises.get(resourceKey.format(resource));
    });
  }
      ...
      try {
        await this.runOperation(async () => {
          ...
          if (locked) {
            ...
            await this.redis.promises.del(this.expirateResource);
          }
        });
      } catch (error) {
  ...
  async expireKeys() {
    ...
    const keys = /** @type {string[]} */ (
      await redisClient.promises.smembers(this.set)
    );
